...
struct ImGuiContext
{
    ...
    bool                    WithinFrameScope;                   // Set by Begin(), cleared by EndFrame()
    bool                    WithinFrameScopeWithImplicitWindow; // Set by Begin(), cleared by EndFrame() when the implicit debug window has been pushed
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImS8                    HiddenFramesForRenderOnly;          // Hide the window until frame N at End() time only
...
namespace ImGui
{
    ...
    // - ImGui::Begin() has never been called, which is illegal.
    // - You are calling ImGui functions after ImGui::EndFrame()/ImGui::End() and before the next ImGui::Begin(), which is also illegal.
    ...
    // End helpers (those functions don't access any ImGui state!)
