...
#define ASSERT(x, msg)                                                         \
  do {                                                                         \
    if (!(x)) {                                                                \
      std::cerr << "Assertion failed! '" << #x << "' at line " << __LINE__     \
...
#define ASSERT_TYPE(type, equals)                                              \
  ASSERT(typeid(type) == typeid(equals), "Type mismatch!")

#define ASSERT_NOT_REACHED(msg)                                                \
  do {                                                                         \
    std::cerr << "Reached invalid line " << __LINE__ << " in file "            \
              << __FILE__ << std::endl;                                        \
    std::cerr << "Reason: '" << (msg) << std::endl;                            \
    __debugbreak();                                                            \
  } while (0)
