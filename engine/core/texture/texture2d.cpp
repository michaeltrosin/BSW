...
bsw::Texture2D::Texture2D(const std::string &path, uint32_t offx, uint32_t offy, uint32_t width, uint32_t height) {
    int w, h, channels;
    ...
    uint32_t array_size = width * height * channels;

    auto *modified_data = new uint8_t[array_size];

    uint32_t index = PIXEL(offx, offy, width) * channels;

    for (uint32_t i = 0; index < array_size; i++, index++) { modified_data[i] = data[index]; }
    ...
    glTextureSubImage2D(m_texture_id, 0, 0, 0, m_width, m_height, data_format, GL_UNSIGNED_BYTE, modified_data);
