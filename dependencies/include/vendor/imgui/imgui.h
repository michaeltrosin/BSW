...
namespace ImGui
{
    ...
    IMGUI_API void          NewFrame();                                 // start a new Dear ImGui frame, you can submit any command from this point until End()/EndFrame().
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by Render(). If you don't need to render data (skipping rendering) you may call EndFrame() without End()... but you'll have wasted CPU already! If you don't need to render, better to not create any windows and not call Begin() at all!
    IMGUI_API void          Render();                                   // ends the Dear ImGui frame, finalize the draw data. You can then get call GetDrawData().
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after End() and until the next call to Begin(). this is what you have to render.
    ...
    IMGUI_API void          PushStyleColor(ImGuiCol idx, ImU32 col);                        // modify a style color. always use this if you modify the style after Begin().
    IMGUI_API void          PushStyleColor(ImGuiCol idx, const ImVec4& col);
    IMGUI_API void          PopStyleColor(int count = 1);
    IMGUI_API void          PushStyleVar(ImGuiStyleVar idx, float val);                     // modify a style float variable. always use this if you modify the style after Begin().
    IMGUI_API void          PushStyleVar(ImGuiStyleVar idx, const ImVec2& val);             // modify a style ImVec2 variable. always use this if you modify the style after Begin().
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor type, reset in ImGui::Begin(), this is updated during the frame. valid before End(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
    IMGUI_API void          SetMouseCursor(ImGuiMouseCursor cursor_type);                       // set desired cursor type
    IMGUI_API void          CaptureMouseFromApp(bool want_capture_mouse_value = true);          // attention: misleading name! manually override io.WantCaptureMouse flag next frame (said flag is entirely left for your application to handle). This is equivalent to setting "io.WantCaptureMouse = want_capture_mouse_value;" after the next Begin() call.
    ...
    IMGUI_API void          LoadIniSettingsFromDisk(const char* ini_filename);                  // call after CreateContext() and before the first call to Begin(). NewFrame() automatically calls LoadIniSettingsFromDisk(io.IniFilename).
    IMGUI_API void          LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size=0); // call after CreateContext() and before the first call to Begin() to provide .ini data from your own data source.
...
// Keyboard: Set io.ConfigFlags |= ImGuiConfigFlags_NavEnableKeyboard to enable. Begin() will automatically fill io.NavInputs[] based on your io.KeysDown[] + io.KeyMap[] arrays.
// Gamepad:  Set io.ConfigFlags |= ImGuiConfigFlags_NavEnableGamepad to enable. Backend: set ImGuiBackendFlags_HasGamepad and fill the io.NavInputs[] fields before calling Begin(). Note that io.NavInputs[] is cleared by EndFrame().
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_NavEnableKeyboard      = 1 << 0,   // Master keyboard navigation enable flag. Begin() will automatically fill io.NavInputs[] based on io.KeysDown[].
    ...
    ImGuiConfigFlags_NoMouse                = 1 << 4,   // Instruct imgui to clear mouse position/buttons in Begin(). This allows ignoring the mouse information set by the backend.
...
// [SECTION] ImGuiStyle
//-----------------------------------------------------------------------------
// You may modify the ImGui::GetStyle() main instance during initialization and before Begin().
...
struct ImGuiIO
{
    ...
    ImFont*     FontDefault;                    // = NULL           // Font to use on Begin(). Use NULL to uses Fonts->Fonts[0].
    ...
    //------------------------------------------------------------------
    // Input - Fill before calling Begin()
    //------------------------------------------------------------------
    ...
    float       NavInputs[ImGuiNavInput_COUNT]; // Gamepad inputs. Cleared back to zero by EndFrame(). Keyboard keys will be auto-mapped and be written here by Begin().
    ...
    //------------------------------------------------------------------
    // Output - Updated by Begin() or EndFrame()/End()
    // (when reading from the io.WantCaptureMouse, io.WantCaptureKeyboard flags to dispatch your inputs, it is
    //  generally easier and more correct to use their state BEFORE calling Begin(). See FAQ for details!)
    //------------------------------------------------------------------
    ...
    int         MetricsRenderVertices;          // Vertices output during last call to End()
    int         MetricsRenderIndices;           // Indices output during last call to End() = number of triangles * 3
    ...
    ImGuiKeyModFlags KeyMods;                   // Key mods flags (same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into flags), updated by Begin()
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect = false);  // End-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
...
struct ImDrawData
{
    bool            Valid;                  // Only valid after End() is called and before the next Begin() is called.
...
struct ImFontAtlas
{
    ...
    bool                        Locked;             // Marked as Locked by ImGui::Begin() so attempt to modify the atlas will assert.
    ...
    ImVector<ImFont*>           Fonts;              // Hold all the fonts returned by AddFont*. Fonts[0] is the default font upon calling ImGui::Begin(), use ImGui::PushFont()/PopFont() to change the current font.
