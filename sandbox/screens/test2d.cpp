...
void Test2D::on_render() {
    ...
    glm::mat4 cam_view = glm::translate(
        glm::mat4(1.0f),
        glm::vec3(window_width() / 4, window_height() / 4, 0));// projection * cam

    glm::mat4 text_cam = glm::translate(
        glm::mat4(1.0f), glm::vec3(window_width() / 4, window_height() / 4, 0));
...
//                                     *FontManager::get_font("Title"), white.darken());
    Renderer2D::draw_centered_string(
        "Some small games written in bsw",
        {window_width() / 2, window_height() / 2 + FontManager::default_font->get_string_height("H")},
        *FontManager::default_font, color);
    Renderer2D::draw_quad({250, 250, 1.0f}, {300, 300}, at);
    Renderer2D::end();
}
...
void Test2D::on_event(Event &event) {
    if (event.is<WindowResizeEvent>()) {
        ...
        m_projection = glm::ortho(0.0f, (float) e->get_width(),
                                 (float) e->get_height(), 0.0f, -1.0f, 1.0f);
...
void Test2D::on_show() {
    at = create_ref<Texture2D>("C:\\Users\\Michael\\Pictures\\a.png");
}
